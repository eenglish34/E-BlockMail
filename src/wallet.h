...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 210000 BIT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBIT send
enum ZerocoinSpendStatus {
    ZBIT_SPEND_OKAY = 0,                            // No error
    ZBIT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBIT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBIT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBIT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBIT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBIT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBIT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBIT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBIT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBIT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBIT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBIT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBIT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBIT_SPENT_USED_ZBIT = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZBitAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 210000 * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 210000 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
