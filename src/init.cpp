...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "eblockmail2.conf"));
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the EBM and zEBM money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 34888, 6082));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in EBM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in EBM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in EBM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:34888"));
    ...
    strUsage += HelpMessageOpt("-backupzeblockmail=<n>", strprintf(_("Enable automatic wallet backups triggered after each zEblockmail minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeeblockmailamount=<n>     " + strprintf(_("Keep N EBM anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Civitas Core Developers"), 2018)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Awesome Eblockmail Core Developers"), 2019)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZEblockmailBackups = GetBoolArg("-backupzeblockmail", true);
        pwalletMain->setZEblockmailAutoBackups(fEnableZEblockmailBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizeEblockmailAmount < 2) nAnonymizeEblockmailAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1EBM+1000 == (.1EBM+100)*10
       10EBM+10000 == (1EBM+1000)*10
    */
