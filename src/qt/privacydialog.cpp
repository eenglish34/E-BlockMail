...
#include "zeblockmailcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCATO ought to be enough for anybody." - Bill Gates, 2017
    ui->zCATOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCATOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCATO->setEnabled(false);
        ui->pushButtonMintzCATO->setToolTip(tr("zCATO is currently disabled due to maintenance."));

        ui->pushButtonSpendzCATO->setEnabled(false);
        ui->pushButtonSpendzCATO->setToolTip(tr("zCATO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCATOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCATO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCATO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCATO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCATO in ") +
...
void PrivacyDialog::on_pushButtonSpendzCATO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCATO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCATO
        sendzCATO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCATO
    sendzCATO();
}

void PrivacyDialog::on_pushButtonZEblockmailControl_clicked()
{
    ZEblockmailControlDialog* zEblockmailControl = new ZEblockmailControlDialog(this);
    zEblockmailControl->setModel(walletModel);
    zEblockmailControl->exec();
}

void PrivacyDialog::setZEblockmailControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEblockmailSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCATO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Eblockmail Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCATOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCATO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CATO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCATO</b>";
    ...
    // use mints from zEblockmail selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEblockmailControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEblockmailControlDialog::GetSelectedMints();
    }
    ...
    // Clear zeblockmail selector in case it was used
    ZEblockmailControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEblockmail Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Eblockmail, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEblockmail Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCATO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCATO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCATO "));
    ui->labelzCATOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCATOSyncStatus->setVisible(fShow);
}
