...
void PrivacyDialog::on_pushButtonMintzBIT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBIT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBIT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBIT in ") +
...
void PrivacyDialog::on_pushButtonSpendzBIT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBIT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void PrivacyDialog::on_pushButtonZBitControl_clicked()
{
    ZBitControlDialog* zBitControl = new ZBitControlDialog(this);
    zBitControl->setModel(walletModel);
    zBitControl->exec();
}

void PrivacyDialog::setZBitControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzBIT()
{
    ...
    // Warn for additional fees if amount is not an integer and change as zBIT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BIT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBIT</b>";
    ...
    // use mints from zBit selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbit selector in case it was used
    ZBitControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBit Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bit, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBit Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBIT </b>";
