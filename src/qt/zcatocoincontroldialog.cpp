...
#include "ui_zeblockmailcontroldialog.h"
...
std::list<std::string> ZEblockmailControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEblockmailControlDialog::listMints;

ZEblockmailControlDialog::ZEblockmailControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEblockmailControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEblockmailControlDialog::~ZEblockmailControlDialog()
{
    delete ui;
}

void ZEblockmailControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEblockmailControlDialog::updateList()
{
...
void ZEblockmailControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEblockmailControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEblockmail_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEblockmailControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEblockmailControlDialog::GetSelectedMints()
{
...
void ZEblockmailControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
