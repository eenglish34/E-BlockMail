...
#include "ui_zbitcontroldialog.h"
...
std::list<std::string> ZBitControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitControlDialog::listMints;

ZBitControlDialog::ZBitControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitControlDialog::~ZBitControlDialog()
{
    delete ui;
}

void ZBitControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitControlDialog::updateList()
{
...
void ZBitControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBit_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitControlDialog::GetSelectedMints()
{
...
void ZBitControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
