...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"bitmoneyprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"bitmoneyprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"bitmoneyaddress\"\n"
            "\nReveals the private key corresponding to 'bitmoneyaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"bitmoneyaddress\"   (string, required) The bitmoney address for the private key\n"
            "\nResult:\n"
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Bitmoney %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"bitmoneyaddress\"\n"
            "\nEncrypts a private key corresponding to 'bitmoneyaddress'.\n"
            "\nArguments:\n"
            "1. \"bitmoneyaddress\"   (string, required) The bitmoney address for the private key (you must hold the key already)\n"
