...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"bitmoneyaddress\"        (string) bitmoney address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH (Value& input, inputs) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Bitmoney address: ") + input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the bitmoney address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
